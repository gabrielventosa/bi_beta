...
jQuery.fn.googleMaps = function(options) {
	...
	return this.each(function() {
		// Create Map
		var gMap = new GMap2(this,opts);
		$(this).data('gMap',gMap);
		$.googleMaps.gMap = gMap;
...
$.googleMaps = {
	mapsConfiguration: function(opts) {
		...
		if ( opts.geocode ) {
			...
			geocoder.getLatLng(opts.geocode, function(center) {
				if (!center) {
					alert(opts.geocode + " no se encontro");
					center = new GLatLng($.googleMaps.defaults.latitude, $.googleMaps.defaults.longitude);
    	      				$.googleMaps.gMap.setCenter(center, opts.depth);
					$.googleMaps.latitude = center.lat();
					$.googleMaps.longitude = center.lng();
				}
				else {
    	      		$.googleMaps.gMap.setCenter(center, opts.depth);
					$.googleMaps.latitude = center.lat();
	...
	mapMarkerLatLong: function(latitude, longitude){
		alert($.googleMaps.marker[0]);	
	},
	...
	mapMarkers: function(center, markers) {
		...
		for ( i = 0; i<markers.length; i++) {
			...
			if (!markers[i].geocode && !markers[i].latitude) {
			...
			if (markers[i].listener) {
				GEvent.addListener($.googleMaps.marker[i], markers[i].eventname, markers[i].controller);
			}
